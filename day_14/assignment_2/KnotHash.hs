...
knothash str = concatMap (printf "%02x") dense
  where
    input = (++ [17, 31, 73, 47, 23]) . map Char.ord $ str
    KnotHashState vec _ _ = knothashInternal 256 input 64
...
knothashInternal :: Int -> [Int] -> Int -> KnotHashState
knothashInternal size reverses times =
